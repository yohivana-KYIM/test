...
const emit = defineEmits(['close', 'videoUpdated']);
const loading = ref(true); // Ajout d'une variable de chargement
...
watch(
  ...
  async (newId) => {
    if (newId) {
      ...
      try {
        const fetchedVideo = await VideoService.getVideoById(newId);
        video.value = {
          title: fetchedVideo.data.title,
          youtube_id: fetchedVideo.data.youtube_id,
           date: fetchedVideo.data.date ? new Date(fetchedVideo.data.date).toISOString().split('T')[0] : null,
        };
      } catch (error) {
        console.error('Error fetching video for edit:', error);
        Swal.fire({
          title: 'Error!',
          text: 'Échec du chargement de la vidéo pour modification.',
          icon: 'error',
        });
      } finally {
        loading.value = false; // Arrêter le chargement, quel que soit le résultat
      }
    }
  },
  { immediate: true }
...
const updateVideo = async () => {
  try {
    await VideoService.updateVideo(props.videoId, video.value);

    Swal.fire({
      toast: true,
      position: 'top-end',
      icon: 'success',
      title: 'Vidéo mise à jour avec succès',
    ...
    emit('close');
     ...
     console.error('Error updating video:', error)
        let message = "Échec de la mise à jour de la vidéo, veuillez réessayer."
