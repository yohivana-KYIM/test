    ...
    <div class="min-h-full px-4 py-8">
      <div class="mx-auto bg-white shadow-2xl dark:bg-gray-800 rounded-xl max-w-7xl">
        <div class="p-6">
          ...
          <!-- Form -->
          <form @submit.prevent="createSubscriber" class="space-y-6">
...
const emit = defineEmits(['close', 'subscriberCreated']);

const createSubscriber = async () => {
  try {
    await SubscriberService.createSubscriber(subscriber.value)
    subscriber.value = {
      email: '',
    }
    Swal.fire({
      ...
      position: 'top-end',
      icon: 'success',
      title: 'Abonné ajouté avec succès',
      showConfirmButton: false,
      timer: 3000,
    })
    emit('subscriberCreated')
    emit('close')
  } catch (error) {
    console.error('Error creating subscriber:', error)
    Swal.fire({
      title: "Erreur!",
      text: error.response?.data?.errors?.email?.[0] || "Impossible d'ajouter l'abonné, veuillez réessayer.",
      icon: "error",
    })
  }
}
