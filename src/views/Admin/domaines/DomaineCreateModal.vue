    ...
    <div class="min-h-full px-4 py-8">
      <div class="mx-auto bg-white shadow-2xl dark:bg-gray-800 rounded-xl max-w-7xl">
        <div class="p-6">
          ...
          <!-- Form -->
          <form @submit.prevent="createDomaine" class="space-y-6">
...
const emit = defineEmits(['close', 'domaineCreated']);

const createDomaine = async () => {
  try {
    await DomaineService.createDomaine(domaine.value)
    domaine.value = {
      nom: '',
    }
    Swal.fire({
    ...
    Swal.fire({
      title: "Erreur!",
      text: "Impossible de créer le domaine, veuillez réessayer.",
      icon: "error",
    })
  }
}
