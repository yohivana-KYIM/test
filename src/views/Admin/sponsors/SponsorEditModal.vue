    ...
    <div class="min-h-full py-8 px-4">
      ...
      >
        <div class="p-6">
          ...
          <!-- Form -->
          <form @submit.prevent="updateSponsor" class="space-y-6">
...
const emit = defineEmits(['close', 'sponsorUpdated'])
...
const updateSponsor = async () => {
   try {
      ...
      await SponsorService.updateSponsor(props.sponsorId, formData);
     ...
     Swal.fire({
         ...
         position: 'top-end',
         icon: 'success',
        title: 'Sponsor modifier avec succès',
      ...
      emit('close');
        ...
        console.error('Error creating sponsor:', error)
         let message = "Échec de la modification du sponsor, veuillez réessayer."
        if(error.errors) {
...
const fetchSponsor = async () => {
    try {
         const response = await SponsorService.getSponsorById(props.sponsorId);
          sponsor.value = {
           nom: response.nom,
        ...
        console.error('Error fetching Sponsor:', error);
         Swal.fire({
            title: 'Error!',
            text: "Échec du chargement du sponsor, veuillez réessayer.",
             icon: 'error',
          })
...
watch(()=> props.sponsorId, async (newSponsorId)=>{
 if(newSponsorId){
       await fetchSponsor()
  }
})
