    ...
    <div class="min-h-full py-8 px-4">
      <div class="bg-white dark:bg-gray-800 rounded-xl shadow-2xl max-w-7xl mx-auto">
        <div class="p-6">
          ...
          <!-- Form -->
          <form @submit.prevent="createMission" class="space-y-6">
...
const emit = defineEmits(['close', 'missionCreated']);

const createMission = async () => {
  try {
    await MissionService.createMission(mission.value)
    mission.value = {
      titre: '',
    }
    Swal.fire({
    ...
    Swal.fire({
        title: "Error!",
        text: "Failed to create the mission, please try again.",
        icon: "error",
    })
  }
}
