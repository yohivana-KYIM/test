    ...
    <div class="min-h-full px-4 py-8">
      <div class="mx-auto bg-white shadow-2xl dark:bg-gray-800 rounded-xl max-w-7xl">
        <div class="p-6">
          ...
          <!-- Form -->
          <form @submit.prevent="updateStat" class="space-y-6">
...
import StatService from '../../../services/StatService';  // Ajustez le chemin
...
const stat = ref({ ...props.stat }) // Crée une copie réactive de l'objet stat

const emit = defineEmits(['close', 'statUpdated']);

const updateStat = async () => {
  try {
    await StatService.updateStat(stat.value.id, stat.value)
    Swal.fire({
      toast: true,
      position: 'top-end',
      icon: 'success',
      title: 'Statistique mise à jour avec succès',
    ...
    Swal.fire({
      title: "Erreur!",
      text: "Impossible de mettre à jour la statistique, veuillez réessayer.",
      icon: "error",
    })
  }
}
...
onMounted(() => {
  // S'assurer que les valeurs sont mises à jour lors de l'ouverture de la modale
