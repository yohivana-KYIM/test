        ...
        <div class="min-h-full py-8 px-4">
            ...
            >
                <div class="p-6">
                    ...
                    <!-- Form -->
                    <form @submit.prevent="updatePublication" class="space-y-6">
...
const emit = defineEmits(['close', 'publicationUpdated'])
...
watch(() => props.publicationId, async (newId) => {
    if(newId) {
         try {
            const fetchedPublication = await PublicationService.getPublicationById(newId);
             publication.value = {
                title: fetchedPublication.title,
                description: fetchedPublication.description,
                publication_date: fetchedPublication.publication_date,
                photo: null,
                document :null,
             }
             photoPreview.value = fetchedPublication.photo;
             if(fetchedPublication.document) {
                documentName.value = fetchedPublication.document.split('/').pop()
             }
          ...
          console.error('Error fetching publication for edit:', error);
           Swal.fire({
                title: 'Error!',
                text:  'Échec du chargement de la publication pour modification.',
                icon: 'error',
              })
        }
    }
...
const updatePublication = async () => {
    try {
      ...
      await PublicationService.updatePublication(props.publicationId, formData);
        ...
        Swal.fire({
            ...
            position: 'top-end',
            icon: 'success',
            title: 'Publication mise à jour avec succès',
        ...
        console.error('Error updating publication:', error);
          let message = "Échec de la mise à jour de la publication, veuillez réessayer."
            if(error.errors) {
