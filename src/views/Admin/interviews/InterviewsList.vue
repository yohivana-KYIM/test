  ...
  <div class="min-h-screen py-8 bg-gray-50">
    <div class="px-4 mx-auto max-w-7xl sm:px-6 lg:px-8">
      ...
      <div class="p-6 mb-6 bg-white rounded-lg shadow-sm">
        ...
        >
          <div>
            <h2 class="text-3xl font-bold text-gray-900">Interviews</h2>
            <p class="mt-1 text-sm text-gray-600">
              Gérez et suivez toutes vos interviews en un seul endroit
            </p>
          </div>
      ...
      >
        ...
        <p class="mt-1 text-sm text-gray-500">
          Commencez par créer une nouvelle interview.
        </p>
        <button
          @click="openCreateModal"
          ...
          <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              ...
              >
                ...
                <td class="px-6 py-4 whitespace-nowrap">
                  <div class="text-sm text-gray-900">{{ interview.source}}</div>
                </td>
                <td class="px-6 py-4 whitespace-nowrap">
                  <div class="text-sm text-gray-900">{{ interview.date }}</div>
                </td>
    ...
    <InterviewCreateModal
      ...
      @interviewCreated="fetchInterviews"
    />
    <InterviewEditModal
      v-if="isEditModalOpen"
      :interviewId="selectedInterview?.id"
      @close="closeEditModal"
      @interviewUpdated="fetchInterviews"
    />
  </div>
</template>
...
import InterviewEditModal from './InterviewEditModal.vue';
...
const fetchInterviews = async () => {
  try {
    loading.value = true;
    interviews.value = await InterviewService.getAllInterviews();
  } catch (error) {
    console.error('Error fetching interviews:', error);
    Swal.fire({
      title: 'Erreur!',
      text: 'Échec du chargement des interviews. Veuillez réessayer.',
      icon: 'error',
    });
  } finally {
    loading.value = false;
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      try {
        ...
        Swal.fire({
          title: 'Supprimé!',
          text: `L'interview de "${interview.titre}" a été supprimée.`,
          icon: 'success',
        });
      } catch (error) {
        Swal.fire({
          title: 'Erreur!',
          text: error.message || 'Échec de la suppression de l\'interview. Veuillez réessayer.',
          icon: 'error',
        });
      }
    }
