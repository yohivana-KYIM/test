        ...
        <div class="min-h-full py-8 px-4">
            ...
            >
                <div class="p-6">
                    ...
                    <!-- Form -->
                    <form @submit.prevent="createReglementation" class="space-y-6">
...
const emit = defineEmits(['close', 'reglementationCreated'])
...
const createReglementation = async () => {
    try {
        ...
        await ReglementationService.createReglementation(formData)
       ...
       Swal.fire({
            ...
            position: 'top-end',
            icon: 'success',
            title: 'Réglementation créée avec succès',
            showConfirmButton: false,
            timer: 3000,
        })

        emit('reglementationCreated')
        emit('close')
    } catch (error) {
        console.error('Error creating reglementation:', error)
        let message = "Échec de la création de la réglementation, veuillez réessayer."
          if(error.errors) {
