  ...
  <div class="min-h-screen py-8 bg-gray-50">
    <div class="px-4 mx-auto max-w-7xl sm:px-6 lg:px-8">
      ...
      <div class="p-6 mb-6 bg-white rounded-lg shadow-sm">
        ...
        >
          <div>
            <h2 class="text-3xl font-bold text-gray-900">Réglementations</h2>
            <p class="mt-1 text-sm text-gray-600">
              Gérez et suivez toutes vos réglementations en un seul endroit
            </p>
          </div>
      ...
      >
        ...
        <p class="mt-1 text-sm text-gray-500">
          Commencez par créer une nouvelle réglementation.
        </p>
        <button
          @click="openCreateModal"
        ...
        <div class="overflow-x-auto">
          <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              <tr
                v-for="reglementation in reglementations"
              ...
              >
                ...
                <!-- <td class="px-6 py-4 whitespace-nowrap">
                   <img
                    v-if="reglementation.image"
                    :src="reglementation.image"
                    :alt="reglementation.title"
                ...
                >
                  <div class="flex justify-end gap-2">
                    <button
                      @click="openEditModal(reglementation)"
                    ...
                    <button
                      @click="deleteReglementation(reglementation)"
                      class="text-red-600 transition-colors duration-200 hover:text-red-900"
                      title="Supprimer la réglementation"
                    >
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        class="w-5 h-5"
    ...
    <!-- Modals -->
    <ReglementationCreateModal
      v-if="isCreateModalOpen"
      @close="closeCreateModal"
      @reglementationCreated="fetchReglementations"
    />
    <ReglementationEditModal
      v-if="isEditModalOpen"
      :reglementationId="selectedReglementation?.id"
      @close="closeEditModal"
      @reglementationUpdated="fetchReglementations"
    />
  </div>
</template>
...
import ReglementationCreateModal from './ReglementationCreateModal.vue'
import ReglementationEditModal from './ReglementationEditModal.vue'

const reglementations = ref([])
const loading = ref(true)
const isCreateModalOpen = ref(false)
const isEditModalOpen = ref(false)
const selectedReglementation = ref(null)
...
const fetchReglementations = async () => {
  try {
    ...
    Swal.fire({
      title: 'Erreur!',
      text: 'Échec du chargement des réglementations. Veuillez réessayer.',
      icon: 'error',
    })
  } finally {
    loading.value = false
...
const openEditModal = (reglementation) => {
  selectedReglementation.value = reglementation
  isEditModalOpen.value = true
}

const closeEditModal = () => {
  isEditModalOpen.value = false
  selectedReglementation.value = null
}

const deleteReglementation = async (reglementation) => {
  Swal.fire({
    title: 'Êtes-vous sûr?',
    ...
    if (result.isConfirmed) {
      try {
        await ReglementationService.deleteReglementation(reglementation.id)
        await fetchReglementations()
        Swal.fire({
          title: 'Supprimé!',
          text: `La réglementation "${reglementation.title}" a été supprimée.`,
          icon: 'success',
        })
      } catch (error) {
        Swal.fire({
          ...
          text:
            error.message ||
            'Échec de la suppression de la réglementation. Veuillez réessayer.',
          icon: 'error',
        })
      }
    }
